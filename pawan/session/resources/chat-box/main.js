...
chat.init = function (myinfo, common, handles) { 
	...
	_d_login.then(	//done callback
			...
			///			         delete_room(room_id).then(function(){ _d.resolve();  },function(){ _d.resolve();  })		
							_d.resolve(); 
...
chat.init_user = function (user) { 
	...
	var uname 	= {},	//received as argument
	    ...
	    user_token 	= {};
	//create a user 
	uname 	= user;//create_username( 'vcuser' ) ;
...
//extra code 
	/*
	 */
/*	rest.get( root_url +  '/login')
			.on('success',function(result, response){
				console.log( response );
				if( response && response.headers ){
					log.info('chal','to rha h');
					temp_cookie = JSON.stringify( response.headers['set-cookie'] );
					get_was_successful = true;
				}		
				log.info('success','get');	
			})
			.on('failure', function( result, response ){
				_d.reject('Login request to ' + root_url + ' failed' );			
			});
			/*.on('complete', function(result, response){
				log.info('chat-box','complete called');
				if( _d.state() == 'pending' ){
					_d.reject('Chat-Login Failed');
				}
			*/
