define(['require', './ot-wrap'], function(require) {
  ...
  var log = require('log')('otl', 'info');

  var otd = {};
  var container;
  var opts = {};

  otd.initLayoutContainer = function(cont, scont, options) {

    // NOTE: internal OT.$ API
    options = OT.$.defaults(options || {}, {
      maxRatio: 3/2,
            minRatio: 9/16,
            fixedRatio: false,
            animate: false,
    ...
    // NOTE: internal OT.$ API
    cont = typeof(cont) == 'string' ? OT.$(cont) : cont;
    scont = typeof(scont) == 'string' ? OT.$(scont) : scont;
    ...
    OT.onLoad(function() {
      // SS comment layout(container, opts);
    });
    return {
      layout_p: otd.layout.bind(null, cont, opts),
        layout_s: otd.layout.bind(null, scont, opts)
    };
  };

  otd.layout = function layout(container, opts, fixedRatio) {
    //otd.layout = function layout() {
    // NOTE: internal OT.$ API
    ...
    var Height = getHeight(container) -
      ...
      getCSSNumber(container, 'paddingTop') -
      getCSSNumber(container, 'paddingBottom');
    ...
    var bigOffsetTop = 0;
    var bigOffsetLeft = 0;
    var bigOnes = Array.prototype.filter.call(
        container.querySelectorAll('#' + id + '>.' + opts.bigClass),
        filterDisplayNone);

    var smallOnes = Array.prototype.filter.call(
        container.querySelectorAll('#' + id + '>*:not(.' + opts.bigClass + ')' + ':not(.avpmenu)'),
          filterDisplayNone);
  ...
  var arrange = function arrange(children, Width, Height, offsetLeft, offsetTop, fixedRatio, minRatio, maxRatio, animate) {
    ...
    for (var i=0; i < children.length; i++) {
      ...
      var actualHeight = vidRatio.targetHeight -
