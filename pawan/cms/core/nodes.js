...
nodes.get_by_dir = function( info, cb){
	/* return whose dir has info.dir as their substr */
	log.debug(' dir is : ' + info.dir);
	Node.find({ 'dir' : new RegExp(info.dir,'i') }, function( err, nodes){
		if( err){
...
function get_node( _node, cb){
	_node.host = _node.host || _node.uid;
	Node.findOne({ 
			'host'	: _node.host, 
			'dir'	: _node.dir, 
			'name'	: _node.name
